...
Page({
  ...
  createFood:function(){
    ...
    ground[x][y]=2
   // console.log(ground)
    // console.log(this.data.snake[i][1])
...
tapEnd:function(){
 ...
 if(Math.abs(heng)>5||Math.abs(zong)>5){
   var direction=Math.abs(heng)>Math.abs(zong)?
   this.computerDir(1,heng):this.computerDir(0,zong)
   switch(direction){
     case"left":
...
changeBottom:function(){
 ...
 checkGame:function(){
   var arr = this.data.snake;
   var len =this.data.snake.length;
   var snakeHEAD = arr[len-1]//【0,2】
   var snakeTAIL=arr[0]
   /* 撞墙，0、22、28  坐标等于这些这触壁，停止运动*/
   if(snakeHEAD[0]<0||snakeHEAD[0]>=this.data.rows||snakeHEAD[1]<0
